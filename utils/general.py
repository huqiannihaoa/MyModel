...
def non_max_suppression(
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        # i = NMS(boxes, scores, iou_thres, class_nms='EfficiCIoULoss')
        # i = NMS(boxes, scores, iou_thres, class_nms='EfficiCIoULoss')
...
# def NMS(boxes, scores, iou_thres, class_nms='EfficiCIoULoss'):
#     # class_nms=class_nms
#     CIoU=EfficiCIoULoss=False
#     if class_nms == 'CIoU':
#         CIoU=True
#     elif class_nms == 'EfficiCIoULoss':
#         EfficiCIoULoss=True
#     else :
#         print('none')
#     B = torch.argsort(scores, dim=-1, descending=True)
...
#         keep.append(index)
#         if B.numel() == 1: break
#         iou = bbox_iou_eciou(boxes[index, :], boxes[B[1:], :], x1y1x2y2=False, EfficiCIoULoss=EfficiCIoULoss, CIoU=CIoU)
#         inds = torch.nonzero(iou <= iou_thres).reshape(-1)
#         B = B[inds + 1]
#     return torch.tensor(keep)


def box_iou_repulsion(box1, box2, x1y1x2y2=True):
...
def NMS(boxes, scores, iou_thres, class_nms='EfficiCIoULoss'):
    ...
    while B.numel() > 0:
        ...
        keep.append(index)
        if B.numel() == 1: break
        iou = bbox_iou_eciou(boxes[index, :], boxes[B[1:], :], x1y1x2y2=False, EfficiCIoULoss=True)
        inds = torch.nonzero(iou <= iou_thres).reshape(-1)
        B = B[inds + 1]
    return torch.tensor(keep)

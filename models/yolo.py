...
class LADH(nn.Module):
    def __init__(self, ch=256, nc=80, anchors=()):
        ...
        self.iou_convs1 = Conv(ch, 256, 3, 1, 1)
        self.iou_convs2 = Conv(256, 128, 3, 1, 1)
        self.iou_convs3 = Conv(128, 64, 3, 1, 1)
...
class BaseModel(nn.Module):
    ...
    def fuse(self):  # fuse model Conv2d() + BatchNorm2d() layers
        ...
        for m in self.model.modules():
            if isinstance(m, (Conv, DWConv,Conv_Mish)) and hasattr(m, 'bn'):
    ...
    def _apply(self, fn):
        ...
        if isinstance(m, (IDetect,Detect, Segment,LADHDetect,ASFF_Detect, Decoupled_Detect)):
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in {
            ...
            BottleneckCSP, C3, C3TR, C3SPP, C3Ghost, nn.ConvTranspose2d,ASPP,SPP,SimSPPF,BasicRFB,SPPCSPC,SPPFCSPC,
            DWConvTranspose2d, C3x, SE, C3SE, C3_SE_Attention,nn.ConvTranspose2d,Shuffle_Block,CBRM,CARAFE,
            GSConv,VoVGSCSP,VoVGSCSPC,BasicStage,AttentionConv,AttentionStem,
            PatchEmbed_FasterNet, PatchMerging_FasterNet,ODConv2d,Dense_C3,CBAM,
            GhostC3,Shuffle_C3,Inception_C3,C3_SimAM, C3_CoT, C3_Double, C3_SK,C3_CBAM,C3_CA,C3_MHSA,
            MobileNetv2_C3,Shuffle_C3,C3_ScSCConv,RepVGGBlock,C2f,CARAFE,C3_Triplet,
            AttentionConv,AttentionStem,BoT3,CoT3,C3_Res2,C3_CoordConv,C3_CoordConv_CA,CoordAtt,EMA_ATT,DCnv2,CoordConv,RepConv}:
            c1, c2 = ch[f], args[0]
            if c2 != no:  # if not output
                c2 = make_divisible(c2 * gw, 8)

            args = [c1, c2, *args[1:]]
            if m in {BottleneckCSP, C3, C3TR, C3Ghost, C3x, C3SE, C3_SE_Attention,VoVGSCSP,VoVGSCSPC,
                     Dense_C3,C3_CA,C3_ScSCConv,C2f,C3_Triplet,CoT3,BoT3,C3_DynamicConv,C3_Res2,C3_CoordConv,C3_CoordConv_CA}:
        ...
        # 添加bifpn_add结构
        # elif m is GConcat_bifpn:
        #     c2 = sum(ch[x] for x in f)
        elif m in [BiFPN_Add2, BiFPN_Add3,GAdd_bifpn]:
            c2 = max([ch[x] for x in f])
        # TODO: channel, gw, gd
        elif m in {IDetect,Detect, Segment,ASFF_Detect, Decoupled_Detect,LADHDetect}:
            ...
            c2 = ch[f] * args[0] ** 2
        elif m is SubPixelConvolution_s:
            c2=ch[f]
            args=[c2,*args]
        elif m is space_to_depth:#添加SPD
            c2 = 4 * ch[f]
        elif m is Expand:
            c2 = ch[f] // args[0] ** 2
        else:
